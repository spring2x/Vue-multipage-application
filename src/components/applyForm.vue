<template>
  <div class="apply-form-box">
    <form class="apply-form" v-if="!isResult">
      ...
      <div class="input-wrap">
        <div class="input-label">机构名称*</div>
        <input v-model="formdata.OrganizationName" type="text" fv-required fv-requiredmsg="必填项">
      </div>
      <div class="input-wrap">
    ...
    methods: {
      submitForm(e){
        ...
        $(e.target).parents('form').trigger('submit', function(res){
          ...
          if(!res.type){
            self.isAjax = true;
            axios.post("/api/submit_2017/submit", Object.assign({}, self.formdata, {ChannelManagementId: self.from}))
            .then(res => {
              self.isAjax = false;
              if(res.status !== 200){
